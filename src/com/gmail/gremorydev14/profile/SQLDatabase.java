...
public class SQLDatabase {
	...
	public SQLDatabase(Player p) {
		...
		try {
			PreparedStatement ps = connection.prepareStatement("SELECT name FROM g_profile WHERE uuid = ?");
			ps.setString(1, p.getUniqueId().toString());
			ResultSet rs = ps.executeQuery();
			if (!rs.next()) {
				ps.close();
				String sql = "INSERT INTO g_profile (uuid, name, achievements_points, xp, level, booster, time, levels)";
				...
				ps.setInt(5, 1);
				ps.setString(6, "");
				ps.setString(7, "0:" + System.currentTimeMillis() / 1000);
	...
	public Object get(String path) {
		Object obj = null;
		try {
			PreparedStatement ps = connection.prepareStatement("SELECT " + path + " FROM g_profile WHERE uuid = ?");
