...
public class SQLDatabase {
	...
	public SQLDatabase(Player p) {
		...
		try {
			PreparedStatement ps = connection.prepareStatement("SELECT name FROM g_sw WHERE uuid = ?");
			ps.setString(1, p.getUniqueId().toString());
			ResultSet rs = ps.executeQuery();
			if (!rs.next()) {
				ps.close();
				String sql = "INSERT INTO g_sw (uuid, name, kSolo, kTeam, kMega, wSolo, wTeam, wMega, souls, options)";
				...
				ps.setInt(8, 0);
				ps.setInt(9, 0);
				ps.setString(10, "default;default;default : normal : normal;normal;normal : false : true");
				ps.executeUpdate();
				ps.close();
			}
			rs.close();
	...
	public Object get(String path) {
		Object obj = null;
		try {
			PreparedStatement ps = connection.prepareStatement("SELECT " + path + " FROM g_sw WHERE uuid = ?");
